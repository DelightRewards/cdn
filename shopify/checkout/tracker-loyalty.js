...
const sendPayloadToBackend = async (data) => {
  const url =
    "https://us-central1-spherical-gate-286620.cloudfunctions.net/loyalty";
  await fetch(url, {
    method: "POST",
    mode: "no-cors",
    headers: {
  ...
  localStorage.setItem("delight-post", newTracker);
};
// Grab info from checkout necessary for referral details
window.addEventListener("load", async () => {
  const tracked = localStorage.getItem("delight-post");
  if (tracked) {
